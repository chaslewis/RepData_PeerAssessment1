...
The mean total number of steps taken per day is **`r format(round(meansteps,1), scientific=F, digits=8)`** while the median is **`r mediansteps`**.  The mean and median are essentially the same; a slight "jitter" had been added to the median in the plot in order to show it next to the mean; otherwise, the line drawn last would be the only one visible.
...
The interval with the maximum average number of steps occurs at **`r maxint$interval`** with a value of **`r round(maxint$avgsteps,2)`** steps.  This result agrees with the prominent morning peak in the plot above (which, we can speculate, reflects the subject's daily routine, perhaps his/her commute to work or morning fitness activity).
...
In this way, we have assembled a complete series of **`r nrow(activity.fix)`** observations from the **`r nrow(activity.clean)`** complete observations and the **`r nrow(activity.plug)`** averages substituted for missing values.
...
The sum of the average intervals is **`r format(round(interval.avgs.sum,1), scientific=F, digits=8)`**, which is (unsurprisingly) the same as the average of the intervals summed by day.  The conclusion is that this approach to filling in missing data does nothing to change the overall analysis, since the estimate of the total daily number of steps is the same; it will, however, tend to reduce the effect analyzed in the next part, weekday vs. weekend activity levels, since plugging average values is insensitive to weekday vs. weekend.  A more sophisticated fill-in approach might take this difference into account use weekend or weekday averages of known values depending on the day being filled in.
...
On weekdays, the subject took an average of **`r format(round(weekday.avg,1), scientific=F, digits=8)`** steps, while his/her weekend average was **`r format(round(weekend.avg,1), scientific=F, digits=8)`** or **`r round(100*(weekend.avg/weekday.avg -1),1)`%** more.
